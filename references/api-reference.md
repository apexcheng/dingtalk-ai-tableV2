...
- 辅助字段名固定为 `查询标记`
- 如果没有 `查询标记` 字段，可以自动新增，字段类型优先 `text`
- 自动创建 `查询标记` 字段不影响其他业务字段
- 只查询 / 只导出 / 只统计，不等于绝对只读；只要需要在 `filters` 或 `sort` 条件下稳定处理超过 `100` 条数据，仍可技术性回写 `查询标记`
- 只有当用户明确要求“禁止任何写入，包括辅助字段”时，才不能使用 `查询标记` 推进分页；此时应明确提示无法保证 `filters` / `sort` 场景下超过 `100` 条数据的稳定分页处理
...
- 日期过滤不要使用区间条件；不要生成 `greater_equal` + `less_than`，也不要生成 `is_after` / `is_before`
- 日期过滤值只传 `yyyy-mm-dd`，不要传完整时间
- 日期范围查询一律按天拆成多次 `eq` 查询；例如查询 `2025-05-01 ~ 2025-05-05`，应依次过滤 `2025-05-01`、`2025-05-02`、`2025-05-03`、`2025-05-04`、`2025-05-05`
- 单日查询达到 `100` 条时，继续在当天范围内使用 `查询标记` 回写推进，不能使用 `cursor`
