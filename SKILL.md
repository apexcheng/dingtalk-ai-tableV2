...
1. 使用 `filters` / `sort` 时，不要依赖 `cursor` 连续翻页。
2. 不要使用“排序 + 过滤 + 翻页”连续遍历。
3. 不要使用“排序 + 翻页”连续遍历。
4. 不要使用“过滤 + 翻页”连续遍历。
5. 批量处理只走“第一页 -> 处理记录 -> 回写辅助标记字段 -> 下次继续查未标记数据”。
6. 如果需要过滤，先过滤获取第一页，处理后回写辅助标记字段，下次继续查询未标记数据。
7. 如果需要排序，也只读取当前排序结果第一页，处理后回写辅助标记字段，不要依赖排序 + 翻页完成全量遍历。
8. 如果表里没有可用的回写字段，Agent 可以自动新增一个辅助字段，例如 `处理标记`、`查询标记`、`同步标记`、`回查标记` 或 `AI处理标记`。
9. 辅助字段只用于避免重复读取和模拟稳定分页，不应承载业务含义。

**过滤字段规则：**

filters 中提供的是字段 ID，不是字段名称。Agent 不能直接把 `商品ID`、`日期`、`店铺` 这类字段名放进过滤条件。
...
4. 使用 fld_product_id 构造 `filters`
...
> 注意：filters 中字段必须使用 **fieldId**，不是 fieldName。`singleSelect` 过滤必须传 option id（通过 `get_fields` 获取）。
> 使用 `filters` / `sort` 时，不要依赖 `cursor` 连续翻页；批量处理请采用“第一页 + 回写辅助标记字段”的推进方式。
