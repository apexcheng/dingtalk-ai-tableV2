...
```json
{
  ...
  "filter": {
    ...
    "operands": [{
      "operator": "between",
      "operands": ["字段fieldId", "2026-05-23T00:00:00+08:00", "2026-05-25T23:59:59+08:00"]
    }]
  }
}
```

> 注意：filter 中字段用 **fieldId**（不是 fieldName）。`singleSelect` 过滤必须传 option id（通过 `get_fields` 获取）。
> 其他操作符：`eq` / `neq` / `contains` / `gt` / `gte` / `lt` / `lte` / `is_empty` / `is_not_empty`。
